...
企业绑定后，「项目概览」提供“企业预警通更新数据”按钮；任何项目参与成员都可更新已绑定企业的数据，复用现有 Task/worker 重新采集六个顶层财务栏目及其下级模块，运行状态在概览显示。搜索企业、首次绑定和更换绑定仍须管理员具备项目编制权限。运行中不能重复点击；未绑定企业时先在财务页选择企业。审计报告的 PDF 查看地址不是财务数据，不再逐期采集或入库；原财务表行列、值和来源 JSON 仍保留。
