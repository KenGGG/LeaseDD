...
2026-09-24 四家公司均完成新版 40 项导入：金银河 31 项有数据 / 9 项原站禁用，德方纳米 30/10，气派科技 29/11，昊志机电 32/8；关键三表可用，导入路径未调用 Agnes。两家公司经历一次定向重试；不能把 40 项记录等同 40 项有数据。详细核验、耗时和限制见 [四家公司验收记录](docs/acceptance/enterprise-warning-four-companies.md)。该结果只证明这四家公司及当前企业预警通页面版本，不代表任意企业或未来页面结构均可兼容。
...
最新自动回归：后端 324 通过 / 3 跳过，前端 47 通过，生产构建成功。主要指标与三表各保存 18 组币种/汇率响应；现有 data 接口按栏目加载并校验批次与摘要。3Y/5Y/10Y 使用滚动报告期窗口，Excel 列数与页面同步；原站指标说明和主要指标趋势弹窗已增量接入。仍待逐项完成全部专项附注交互/导出、三表及分析趋势、不同口径工具栏组合和视觉细节的原站对照；当前耗时没有同企业 PDF + Agnes 对照基线，不能声称已证明加速。持续目标与证据见[六栏目对照记录](docs/acceptance/enterprise-financial-ui-parity.md)和[增量计划](docs/superpowers/plans/2026-09-23-enterprise-financial-parity.md)。
