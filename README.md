...
当前覆盖 21 个模块：主要财务指标、三张财务报表、财务分析 7 项，以及财务附注 10 项。每个模块保留原始 JSON、来源接口、请求参数、期间、单位、口径和响应摘要；空值保持为空，不补零。三张表转换为现有财务 API 和页面结构，七条恒等式只产生质量提示，不修改或阻断来源值。企业预警通导入路径不调用 Agnes，也不会与 PDF 提取结果逐格拼接。
