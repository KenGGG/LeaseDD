...
全站一级导航统一为财务核对页的深蓝顶部栏，覆盖尽调项目、项目概览、资料与证据、财务核对、报告章节、复核与导出及管理员配置；窄屏保持顶部横向滚动。标题、按钮、表单、卡片、面包屑及弹窗采用统一视觉规则，财务页保留页内二级栏目树。不变更数据、权限或业务流程。全站浏览器回归覆盖 1440px／390px 下 7 个页面，另有项目成员更新数据及 18 项附注浏览器回归；完整 Python 测试 372 通过／3 跳过（既有 AnyIO 弃用警告），前端 65 项通过并构建成功。此项不代表企业预警通六栏目的全部交互与导出已完成原站对照。

复跑界面回归：前端服务启动后执行 `LEASEDD_BROWSER_URL=http://127.0.0.1:5173 .venv/bin/python tests/app_navigation_browser.py`。测试仅替换 API 响应，不写入真实项目数据。
...
企业绑定后，「项目概览」提供“企业预警通更新数据”按钮；任何项目参与成员（编制或复核）都可更新已绑定企业的数据，也可在财务页重试失败模块。更新复用现有 Task/worker 重新采集六个顶层财务栏目及其下级模块，运行状态在概览显示。搜索企业、首次绑定和更换绑定仍须管理员具备项目编制权限；非项目成员不能更新或重试。运行中不能重复点击；未绑定企业时先在财务页选择企业。审计报告的 PDF 查看地址不是财务数据，不再逐期采集或入库；原财务表行列、值和来源 JSON 仍保留。
...
附注明细中有来源企业代码的名称可进入原站企业概览；普通坏账附注中已核实的企业类型和负面信息标签可显示、悬停查看并用“标签”开关隐藏。只按可与来源行位核对的字段呈现；旧接口标签轴尚未核实的页面不猜填。六栏目完整视觉和交互对照仍在进行中。
...
当前部署通过本机专用浏览器服务的受限 Unix socket 连接已登录的企业预警通，Docker worker 不挂载浏览器配置。不要把浏览器用户目录、Cookie、密码或认证响应提交到 Git，也不要将个人持久浏览器配置直接挂载到容器。管理员首次绑定或更换企业仍须具备项目写入授权；已绑定企业的更新允许任何项目成员触发。本轮四家公司临时授权在验收后已撤销，不改变原编制/复核成员。
...
主要指标与三表各保存 18 组币种/汇率响应；现有 data 接口按栏目加载并校验批次与摘要。3Y/5Y/10Y 使用滚动报告期窗口，Excel 列数与页面同步；主要指标和三表趋势弹窗已接入，三表趋势只使用保存的披露列，导出复用现有 data 接口。两项前五名应收款已接入原站精确明细接口，但仅在重导入完整批次后才可用于对应项目；旧 F9 批次页面明确标识“非原站精确表”。科学计数法金额在来源核验、网页及 Excel 中按 Decimal 处理，原文不改。仍待逐项完成全部专项附注交互/导出、财务分析趋势、不同口径工具栏组合和视觉细节的原站对照；当前没有同企业 PDF + Agnes 的可比耗时基线。持续目标与证据见[六栏目对照记录](docs/acceptance/enterprise-financial-ui-parity.md)和[增量计划](docs/superpowers/plans/2026-09-23-enterprise-financial-parity.md)。

当前精确附注重导入状态（2026-09-24）：金银河、德方纳米、气派科技、昊志机电均已有 40 项目录的完整批次，分别为 31/9、30/10、29/11、32/8 项有数据/原站禁用，0 项失败；四家公司各两项前五名应收款均来自 `/getData.action` 精确明细，不再使用旧 F9 摘要展示。气派科技重试任务已完成，旧完整批次仍保留，临时管理员项目写权限已撤销并留审计。桥接目录偶发超时已定位为“财务附注”虚拟滚动节点未挂载时直接按文本点击，现改用既有滚动定位；修复后连续 8 次及四家公司各一次只读目录测试均返回 40 项，长期稳定性仍需观察。最新后端自动回归 354 通过/3 跳过；四家公司只读验收见[当前结果](docs/acceptance/enterprise-warning-four-companies-current.json)。结果中的耗时为各公司最后一个任务（可能只重试部分模块）的排队至完成时间，不可当作完整 40 模块首轮采集速度或与 PDF + Agnes 路径的可比结论。德方纳米「货币资金」「存货」「财务费用」Excel 已按实际下载对齐来源行、表头和原行序号；原站导出有与页面原值不一致的单位格式，LeaseDD 保留原文而未照抄错误单位。六栏目全部交互和视觉逐项对照仍未完成，详见[六栏目对照记录](docs/acceptance/enterprise-financial-ui-parity.md)。

2048px 主要指标宽屏布局已按原站截图压缩为单行工具栏和约 220px 栏目树，受控页面 7 期可见、表格起点约 188px；这只是布局回归，非四家公司全部财务页面的完整视觉验收。

附注增量对照：已保存来源 `level` 用于存货/账龄明细缩进，源 `head[0]` 用于区分“项目名称”与“账龄”；八个已核实附注页不再显示原站没有的排序按钮。“货币资金”报告期已选标签可逐个移除，实点原站确认移除“最新”会去掉 2026 年中报列。上述是部分页面和交互核验，其他下级栏目、全部筛选组合、导出及四家公司逐页视觉验收仍未完成。

同一普通附注报告期下拉实点只有“最新、年报、中报”，选择立即生效且无“确定”；已核实的八页按此隐藏多余按钮，财务分析等自定义菜单继续保留原站“确定”操作。完整逐页验收状态仍见六栏目对照记录。

财务分析已重新查看德方纳米原站 6 个可加载的下级页面，移除本系统多出的“隐藏空行”开关；1280px 受控每股指标页由仅 2 期可见改为 6 期可见。盈利能力原站仍间歇性停在“加载中”，其页面交互与六栏目完整对照仍待核实。

排序实点补查确认原站分析页用“报告期降序/正序”，主要指标与三表用“报告期倒序”；本地分析页排序后的表格列与 Excel 下载列已做一致性回归。其它下级栏目和公司口径仍未完成逐项验收。
