...
当前精确附注重导入状态（2026-09-24）：昊志机电新版完整批次已入库，两项前五名应收款均为原站精确明细；金银河、德方纳米、气派科技的新批次因浏览器启动/来源请求故障仍不完整，页面继续读取各自上一完整批次，不能把旧 F9 摘要称为精确明细。主机内存与交换空间接近耗尽，未擅自停止另一个项目 OARadar 的 MinerU；临时授予的四条管理员项目权限已撤销。桥接网络失败分类、有界重试和导入终态修复已部署。最新后端 344 通过/3 跳过，前端 55 通过；新增桌面财务表密度浏览器断言通过。高资源压力下三表趋势下载浏览器回归超时，仍待资源恢复后重跑。详见[六栏目对照记录](docs/acceptance/enterprise-financial-ui-parity.md)。
