...
| 多候选 | 单次响应返回排序后的零到多个候选；完整名称观察到 4 个候选，歧义短名“科技”观察到 15 个候选 |
...
旧验收快照中的账龄分析、前五名、受限资产、财务费用等名称是附注响应内部的表格或明细标题，不是当前导航树中的独立入口。生产适配器应保存模块标识、表内层级和原始标题，不把显示名称当唯一键，也不硬编码所有公司的附注树。
...
| 资产负债表 | `GET …/finance/report/getThreeReports`，以 `childType` 区分 | 一次响应包含多期 | `head/key/level` 与 `value[期间][字段]`；单位/币种由请求参数与 `CUCodeUnCast` 表达 | 无 | 未观察到 | 不需要 |
| 利润表 | 同上，以 `childType` 区分 | 一次响应包含多期 | 同上 | 无 | 未观察到 | 不需要 |
| 现金流量表 | 同上，以 `childType` 区分 | 一次响应包含多期 | 同上 | 无 | 未观察到 | 不需要 |
...
getThreeReports.data = {
  head: string[], key: string[], level: number[], blankNum: number[],
  dataMark: string, CUCodeUnCast: number, value: scalar[][]
}
...
- 17 个当前导航模块（4 个报表/指标、7 个分析、6 个附注）均已逐项触发并取得对应结构化响应。
- 资产负债表、每股指标和审计报告分别在独立新页面重复加载，两次权威响应内容哈希一致；端点不是仅由单页 SPA 缓存提供。
- 三表和财务分析请求带期间类型/年数参数；财务附注的报告类型控件在无头探测中没有展开可选项，因此尚未证明切换是否发出新请求。该限制不影响当前响应已一次返回全部显示期间的结论。
